...
contract Telebets is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    TelebetsDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD

    uint256 public _totalSupply = 50_000_000_000 * (10**18);
    ...
    address payable public _marketingWalletAddress = payable(0x7dD0c2206F60317F99E044347f5CbEEf14223B4f);
    address payable public _buybackWalletAddress = payable(0x1578d1820148bd7ea8Dc4741a8416ccDbcBd4C6C);
    ...
    constructor() BEP20("Telebets", "TLB") 
    {
        dividendTracker = new TelebetsDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Telebets: The dividend tracker already has that address");
        TelebetsDividendTracker newDividendTracker = TelebetsDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Telebets: The new dividend tracker must be owned by the Telebets token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Telebets: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Telebets: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Telebets: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Telebets: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Telebets: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Telebets: Cannot update gasForProcessing to same value");
...
contract TelebetsDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Telebets_Dividend_Tracker", "Telebets_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Telebets_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Telebets_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Telebets contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Telebets_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Telebets_Dividend_Tracker: Cannot update claimWait to same value");
